...
**Development preview.** The ordinary build includes six real applications indexed from Omarchy’s stable x86_64 package repository: OmaCalc, OmaCut, OmaWrite, OmaPresent, LocalSend and Heroic Games Launcher. These community entries have unclaimed makers and no OmaStore runtime test claims. A separate demo build contains six explicitly fictional listings for exercising the interface. The demo also supports fictional author/reviewer/operator sessions and a local publication rehearsal. Real provider integration and Omarchy desktop evidence remain release gates; live package writes remain disabled until actual Omarchy lifecycle evidence is recorded. Selective settings/remixes and the full fictional commerce lifecycle are implemented. Live setting adapters and real payments remain separately gated.

![Actual native Discover page with real repository listings](docs/screenshots/discover.png)

The first storefront polish pass adds purpose browsing, an attributed upstream OmaCalc screenshot and a clearer installation review. [See the app page](docs/screenshots/detail.png) and [installation sheet](docs/screenshots/installation.png). These are actual native offscreen captures; desktop acceptance on Omarchy remains pending.
