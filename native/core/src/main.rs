...
fn local_request(runtime: &mut Runtime, method: &str, params: Value) -> platform::Result<Value> {
    ...
    match method {
        "library.activity" => {
            ...
            store.activity(chrono::Utc::now().timestamp())
        }
        "operations.status" => {
