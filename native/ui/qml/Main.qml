...
ApplicationWindow {
    ...
    Shortcut { sequence: "Alt+Left"; enabled: window.showingSubpage && !planDialog.visible && !settingsDialog.visible; onActivated: window.back() }
    Shortcut { sequence: "Escape"; enabled: window.showingSubpage && !planDialog.visible && !settingsDialog.visible; onActivated: window.back() }
    ...
    Component {id:libraryPage;LibraryPage {core:window.core;theme:storeTheme;onSettingsRequested:settingsDialog.openFor([])}}
    ...
    Component {id:setupsPage;SetupsPage {core:window.core;theme:storeTheme;mediaPreview:window.mediaPreview;selectedId:window.setupSelection;selectedRevision:window.setupRevision;onChosen:(id,revision)=>{window.setupSelection=id;window.setupRevision=revision;};onMakerChosen:(id)=>window.showMaker(id);onSettingsRequested:(references)=>settingsDialog.openFor(references)}}
