...
Implementation constraint: the provider subscription percentage applies to the invoice total. The initial test adapter restricts managed subscriptions to zero tax and exact 5% minor-unit amounts. Unsupported combinations fail before session creation. Tom owns a provider-reviewed replacement for this limitation; external author billing continues. The raw-event API version is checked alongside the signature. A later API version requires an explicit adapter/fixture review.

Lifecycle implementation keeps attributable store proceeds separate from whole-account provider cash and payout observations. Accounting reserves never claim to lock provider funds. Refund fee obligations are allocated cumulatively from the original fee and posted only when separately confirmed. Processing costs and outside-store refund discrepancies remain visible; currency conversion is unsupported pending an explicit reviewed settlement contract.

Recurring invoice cycles retain frozen accepted terms and distinct receipts. Current billing state is separately retrieved, while paid periods come only from exact confirmed invoice charges. Cancellation is an absolute period-end setting, reconciled before any repeat. Full refunds block new recovery/delivery through the refunded receipt, while earlier signed records remain. Hosted revocation and out-of-band commercial adjustments require the appointed operator/provider's confirmed process. No local flag claims that a remote service account was revoked. [Current subscription item periods](https://docs.stripe.com/api/subscriptions/retrieve).
